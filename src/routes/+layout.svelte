<script lang="ts">
	import '../app.css';

	let { children } = $props();

	// State for theme toggle
	let darkMode = $state(true);

	function toggleTheme() {
		darkMode = !darkMode;
		document.documentElement.setAttribute('data-theme', darkMode ? 'synthwave' : 'light');
	}
</script>

<div class="drawer min-h-screen">
	<input id="my-drawer-3" type="checkbox" class="drawer-toggle" />
	<div class="drawer-content flex min-h-screen flex-col">
		<!-- Navbar -->
		<div class="navbar bg-base-200 shadow">
			<div class="navbar-start">
				<div class="flex-none lg:hidden">
					<label for="my-drawer-3" class="btn btn-square btn-ghost">
						<svg
							xmlns="http://www.w3.org/2000/svg"
							fill="none"
							viewBox="0 0 24 24"
							class="inline-block h-6 w-6 stroke-current"
							><path
								stroke-linecap="round"
								stroke-linejoin="round"
								stroke-width="2"
								d="M4 6h16M4 12h16M4 18h16"
							></path></svg
						>
					</label>
				</div>
				<a href="/" class="btn btn-ghost text-xl normal-case">The Card Machine</a>
				<div class="hidden lg:flex">
					<ul class="menu menu-horizontal px-1">
						<li><a href="/" class="text-base-content">Home</a></li>
						<li><a href="/about" class="text-base-content">About</a></li>
						<li>
							<details>
								<summary>Services</summary>
								<ul class="bg-base-200 rounded-box p-2">
									<li><a href="/services/one">Service One</a></li>
									<li><a href="/services/two">Service Two</a></li>
								</ul>
							</details>
						</li>
						<li><a href="/contact">Contact</a></li>
					</ul>
				</div>
			</div>

			<div class="navbar-end">
				<!-- Theme toggle -->
				<label class="swap swap-rotate mr-2">
					<input type="checkbox" checked={darkMode} onchange={toggleTheme} />
					<!-- Sun icon -->
					<svg
						class="swap-off h-6 w-6 fill-current"
						xmlns="http://www.w3.org/2000/svg"
						viewBox="0 0 24 24"
						><path
							d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z"
						/></svg
					>
					<!-- Moon icon -->
					<svg
						class="swap-on h-6 w-6 fill-current"
						xmlns="http://www.w3.org/2000/svg"
						viewBox="0 0 24 24"
						><path
							d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z"
						/></svg
					>
				</label>

				<!-- Notifications -->
				<div class="dropdown dropdown-end">
					<button class="btn btn-ghost btn-circle">
						<div class="indicator">
							<svg
								xmlns="http://www.w3.org/2000/svg"
								class="h-5 w-5"
								fill="none"
								viewBox="0 0 24 24"
								stroke="currentColor"
								><path
									stroke-linecap="round"
									stroke-linejoin="round"
									stroke-width="2"
									d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"
								/></svg
							>
							<span class="badge badge-xs badge-primary indicator-item"></span>
						</div>
					</button>
					<div class="card dropdown-content card-compact bg-base-200 z-[1] mt-3 w-52 shadow">
						<div class="card-body">
							<span class="text-lg font-bold">Notifications</span>
							<span class="text-info">You have 1 unread message</span>
							<div class="card-actions">
								<button class="btn btn-primary btn-block">View all</button>
							</div>
						</div>
					</div>
				</div>

				<!-- Avatar -->
				<div class="dropdown dropdown-end">
					<button class="btn btn-ghost btn-circle avatar">
						<div class="w-10 rounded-full">
							<img alt="Avatar" src="https://picsum.photos/200/200?random=1" />
						</div>
					</button>
					<ul
						class="menu menu-sm dropdown-content bg-base-200 rounded-box z-[1] mt-3 w-52 p-2 shadow"
					>
						<li>
							<a href="/profile" class="justify-between">
								Profile
								<span class="badge badge-primary badge-sm">New</span>
							</a>
						</li>
						<li><a href="/settings">Settings</a></li>
						<li><a href="/logout">Logout</a></li>
					</ul>
				</div>
			</div>
		</div>

		<!-- Main content -->
		<main class="container mx-auto flex-grow p-4">
			<div class="card bg-base-100 shadow-xl">
				<div class="card-body">
					{@render children()}
				</div>
			</div>
		</main>

		<!-- Footer -->
		<footer class="footer footer-center bg-base-300 text-base-content mt-auto p-4">
			<aside>
				<div class="join">
					<button class="join-item btn btn-sm">Facebook</button>
					<button class="join-item btn btn-sm">Twitter</button>
					<button class="join-item btn btn-sm">Instagram</button>
				</div>
				<p class="mt-2">
					Copyright © {new Date().getFullYear()} - The Card Machine - All rights reserved
				</p>
			</aside>
		</footer>
	</div>

	<!-- Drawer side (mobile menu) -->
	<div class="drawer-side">
		<label for="my-drawer-3" aria-label="close sidebar" class="drawer-overlay"></label>
		<ul class="menu bg-base-200 min-h-full w-80 p-4">
			<li><a href="/">Home</a></li>
			<li><a href="/about">About</a></li>
			<li class="menu-title">
				<span>Services</span>
			</li>
			<li><a href="/services/one">Service One</a></li>
			<li><a href="/services/two">Service Two</a></li>
			<li class="divider"></li>
			<li><a href="/contact">Contact</a></li>
		</ul>
	</div>
</div>
